...
`alert-policies.json` contains checked Google `AlertPolicy` objects. The
...
where present, the bounded `outcome` and `action` fields. SSE state is selected
by fixed filters instead of becoming another label.
...
These thresholds are operating hypotheses, not SLOs:
