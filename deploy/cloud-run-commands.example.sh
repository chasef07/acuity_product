...
: "${HUMAN_CALLING_SAFE_VOICEMAIL_GREETING_URL:?required HTTPS audio URL}"
...
case "$HUMAN_CALLING_SAFE_VOICEMAIL_GREETING_URL" in
  https://*) ;;
  *)
    echo "safe voicemail greeting URL must use HTTPS" >&2
    exit 1
    ;;
esac
...
deploy_service "$PORTAL_DATABASE_URL_SECRET" --no-invoker-iam-check \
  "HUMAN_CALLING_HANDOFF_TOKEN_KEY=${HANDOFF_TOKEN_KEY_SECRET}:latest,HUMAN_CALLING_PLAYBACK_SIGNING_KEY=${PLAYBACK_SIGNING_KEY_SECRET}:latest,HANDOFF_SERVICE_TOKEN=${HANDOFF_SERVICE_TOKEN_SECRET}:latest,TELNYX_API_KEY=${TELNYX_API_KEY_SECRET}:latest" \
  "BROWSER_ORIGIN=${BROWSER_ORIGIN},BETTER_AUTH_JWKS_URL=${BETTER_AUTH_JWKS_URL},BETTER_AUTH_ISSUER=${BETTER_AUTH_ISSUER},PORTAL_API_AUDIENCE=${PORTAL_API_AUDIENCE},HUMAN_CALLING_SIP_DOMAIN=${HUMAN_CALLING_SIP_DOMAIN},HUMAN_CALLING_STAFF_SIP_DOMAIN=${HUMAN_CALLING_STAFF_SIP_DOMAIN},HUMAN_CALLING_SAFE_VOICEMAIL_GREETING_URL=${HUMAN_CALLING_SAFE_VOICEMAIL_GREETING_URL},HANDOFF_SERVICE_SUBJECT=${HANDOFF_SERVICE_SUBJECT},HANDOFF_SERVICE_PRACTICE_ID=${HANDOFF_SERVICE_PRACTICE_ID},TELNYX_CALL_CONTROL_ID=${TELNYX_CALL_CONTROL_ID},TELNYX_CREDENTIAL_CONNECTION_ID=${TELNYX_CREDENTIAL_CONNECTION_ID},TELNYX_FROM_NUMBER=${TELNYX_FROM_NUMBER},TELNYX_RINGBACK_URL=${TELNYX_RINGBACK_URL},TELNYX_RECORDING_BUCKET=${TELNYX_RECORDING_BUCKET},MESSAGING_WEBHOOK_BASE_URL=${MESSAGING_WEBHOOK_BASE_URL},MESSAGING_ATTACHMENT_DIRECTORY=${MESSAGING_ATTACHMENT_DIRECTORY},${CALLING_TIMING_ENV}"
