    ...
    IF NEW.closeout_payload ? 'domainOutcomes' OR historical_tools IS NULL THEN
        SELECT EXISTS (
            ...
            WHERE lower(availability_call.item ->> 'type') = 'function_call'
                AND lower(availability_call.item ->> 'name') IN ('get_availability', 'list_available_appointments')
                ...
                AND EXISTS (
                    ...
                    WHERE lower(availability_output.item ->> 'type') = 'function_call_output'
                        ...
                        -- The current tool returns policy/provider failures as ordinary text.
                        AND (lower(availability_call.item ->> 'name') = 'get_availability'
                            OR lower(ltrim(availability_output.item ->> 'output')) LIKE ANY (ARRAY['success:%', 'no_results:%']))
                )
...
-- Recompute already-recorded calls using the renamed tool. The existing
-- trigger remains the owner of conversion and patient-group facts.
UPDATE ai_interactions
SET transcript = transcript
WHERE status <> 'IN_PROGRESS' AND lifecycle_stage = 3
    AND jsonb_path_exists(analytics_evidence,
        '$.transcript.items[*] ? (@.type == "function_call" && @.name == "list_available_appointments")');
