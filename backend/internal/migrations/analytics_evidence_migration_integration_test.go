...
func TestAnalyticsEvidenceBackfillResumesCommittedBatches(t *testing.T) {
	ctx := context.Background()
	pool := testdb.OpenThrough(t, "0054_classify_completed_booking_searches.sql")
	var practiceID, locationID string
	if err := pool.QueryRow(ctx, `INSERT INTO access_practices(provisioning_key,name) VALUES('analytics-backfill','Analytics Backfill') RETURNING id::text`).Scan(&practiceID); err != nil {
		t.Fatal(err)
	}
 ...
 SELECT ('00000000-0000-0000-0000-'||lpad(n::text,12,'0'))::uuid,'agent',$1,$2,'source-'||n,'+15555550101','+15555550102',now(),'IN_PROGRESS',1,'{"items":[{"metrics":{"e2eLatencyMs":400}}]}' FROM generate_series(1,205) AS n`, practiceID, locationID); err != nil {
		t.Fatal(err)
	}
	...
	start := time.Now()
	err = migrations.ApplyThrough(ctx, pool, "0056_backfill_ai_analytics_evidence.sql")
	if err == nil {
	...
	var projected int
	if err := pool.QueryRow(ctx, `SELECT count(*) FROM ai_interactions WHERE analytics_evidence IS NOT NULL`).Scan(&projected); err != nil {
		t.Fatal(err)
	}
	if projected != 100 {
	...
	if err := pool.QueryRow(ctx, `SELECT count(*) FROM ai_interactions WHERE analytics_evidence #>> '{transcript,items,0,metrics,e2eLatencyMs}' = '400'`).Scan(&projected); err != nil {
		t.Fatal(err)
	}
	if projected != 205 {
