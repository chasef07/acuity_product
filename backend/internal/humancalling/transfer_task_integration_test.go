...
func TestTransferredAITaskSurvivesMissedCallAndLateVoicemail(t *testing.T) {
	for _, completeBeforeVoicemail := range []bool{false, true} {
		t.Run(fmt.Sprintf("completed=%t", completeBeforeVoicemail), func(t *testing.T) {
 ...
 (SELECT count(*) FROM human_calling_handoffs), task_id::text, outcome, audio_state FROM human_calling_voicemails`).Scan(&tasks, &attachments, &handoffs, &voicemailTask, &outcome, &audioState); err != nil {
			...
			if final.State != completed.State || final.Version != expectedVersion || final.Title != task.Title || final.SourceMessage != task.SourceMessage {
				t.Fatalf("late evidence changed original work: %#v", final)
			}

			if !completeBeforeVoicemail {
				_, err = module.CompleteTask(ctx, work.CompleteTaskCommand{Identity: staff[0], TaskID: taskID, ExpectedVersion: task.Version})
				if !errors.Is(err, work.ErrConflict) {
					t.Fatalf("stale single completion accepted: %v", err)
				}
				_, err = module.CompleteTaskGroup(ctx, work.CompleteTaskGroupCommand{Identity: staff[0], TaskID: taskID, Members: []work.ReviewedTask{{ID: taskID, ExpectedVersion: task.Version}}})
				if !errors.Is(err, work.ErrConflict) {
					t.Fatalf("stale grouped completion accepted: %v", err)
				}
			}
