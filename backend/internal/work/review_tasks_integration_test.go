...
func TestAppointmentReviewIdentitySurvivesCompletionAndNewOutcome(t *testing.T) {
	ctx := context.Background()
	pool := testdb.Open(t)
	now := time.Now().UTC().Truncate(time.Microsecond)
	a := access.New(pool, func() time.Time { return now })
	...
	at := now.Add(-30 * 24 * time.Hour)
	ensure := func(action string, at time.Time) {
		t.Helper()
		tx, err := pool.Begin(ctx)
		if err != nil {
			t.Fatal(err)
		...
		defer tx.Rollback(ctx)
		if err := m.EnsureAppointmentReview(ctx, tx, sourceID, auth.Practice.ID, auth.Locations[0].ID, "+15555550123", "synthetic-call", action, "Verify synthetic appointment provider and insurance.", at); err != nil {
			t.Fatal(err)
		}
		if err := tx.Commit(ctx); err != nil {
			t.Fatal(err)
	...
	var id string
	if err := pool.QueryRow(ctx, `SELECT id::text FROM work_tasks WHERE origin='APPOINTMENT_REVIEW'`).Scan(&id); err != nil {
		t.Fatal(err)
	}
	original, err := m.ReadTask(ctx, identity, id)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := m.CompleteTask(ctx, work.CompleteTaskCommand{Identity: identity, TaskID: id, ExpectedVersion: original.Version}); err != nil {
		t.Fatal(err)
	}
	ensure("BOOKED", at)
	ensure("CANCELLED", at.Add(time.Hour))
	var total, open int
	if err := pool.QueryRow(ctx, `SELECT count(*),count(*) FILTER(WHERE state='OPEN') FROM work_tasks WHERE origin='APPOINTMENT_REVIEW'`).Scan(&total, &open); err != nil || total != 2 || open != 1 {
		t.Fatalf("outcome replay/new review: %d %d %v", total, open, err)
	}
	if _, err := pool.Exec(ctx, `UPDATE work_tasks SET source_review_key='changed' WHERE id=$1`, id); err == nil {
		t.Fatal("review source key was mutable")
	}
}
