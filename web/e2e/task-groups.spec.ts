...
test("staff review grouped requests, correct one group, and preserve completed requests", async ({ page }, testInfo) => {
  ...
  await page.reload()
  const taskView = page.getByRole("button", { name: "Filter My Tasks", exact: true })
  ...
  await page.getByRole("button", { name: "Filter My Tasks · Optical", exact: true }).click()
  await page.getByRole("menuitemradio", { name: "All Tasks", exact: true }).click()
  await page.getByRole("button", { name: "Filter All Tasks · Optical", exact: true }).click()
  await page.getByRole("menuitemradio", { name: /^Optical / }).click()
  const allTasksOptical = page.getByRole("button", { name: "Filter All Tasks · Optical", exact: true })
