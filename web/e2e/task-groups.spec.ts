...
test("staff review grouped requests, correct one group, and retain feedback after Resolve group", async ({ page }, testInfo) => {
  ...
  const taskView = page.getByRole("button", { name: "Task view: My tasks, All types", exact: true })
  ...
  await page.getByRole("menuitem", { name: "View: My tasks", exact: true }).click()
  ...
  await page.getByRole("button", { name: "Task view: My tasks, Optical", exact: true }).click()
  await page.getByRole("menuitem", { name: "View: My tasks", exact: true }).click()
  await page.getByRole("menuitemradio", { name: "All tasks", exact: true }).click()
  await expect(page.getByRole("button", { name: "Task view: All tasks, All types", exact: true })).toBeVisible()
  await expect(page.getByTestId("task-row").filter({ hasText: "Group request medication" })).toBeVisible()
  await page.reload()
