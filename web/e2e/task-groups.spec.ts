...
test("staff review grouped requests, correct one group, and retain feedback after Resolve group", async ({ page }, testInfo) => {
  ...
  await page.getByLabel("Knowledge flagged", { exact: true }).check()
  const flagged = page.getByTestId("task-row").filter({ hasText: "Group request glasses" })
  await expect(flagged).toBeVisible()
  await flagged.getByRole("button", { name: "Group request glasses", exact: true }).click()
  await expect(page.getByLabel("Suggested answer (optional)")).toHaveValue("Confirm the prescription-copy delivery process with the office.")
