...
test("staff review grouped requests, correct one group, and retain feedback after Resolve group", async ({ page }, testInfo) => {
  ...
  const taskView = page.getByRole("button", { name: "Task view: My groups, All types", exact: true })
  ...
  await page.getByRole("menuitem", { name: "View: My groups", exact: true }).click()
  await expect(page.getByRole("menuitemradio", { name: "My groups", exact: true })).toHaveAttribute("aria-checked", "true")
  await page.getByRole("menuitemradio", { name: "My groups", exact: true }).click()
  await expect(page.getByRole("menuitemradio", { name: "My groups", exact: true })).toBeHidden()
  ...
  await page.getByRole("button", { name: "Task view: My groups, Optical", exact: true }).click()
  await page.getByRole("menuitem", { name: "View: My groups", exact: true }).click()
  await page.getByRole("menuitemradio", { name: "All tasks", exact: true }).click()
  const allTasksOptical = page.getByRole("button", { name: "Task view: All tasks, Optical", exact: true })
