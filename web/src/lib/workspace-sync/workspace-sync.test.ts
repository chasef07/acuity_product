...
test("a hidden reconnect stays degraded until its visible catch-up succeeds", async () => {
  ...
  const sync = createWorkspaceSync({
    ...
    timing: {
      ...
      pollMinimumMilliseconds: 1_000,
      pollMaximumMilliseconds: 2_000,
    },
  })
  ...
  try {
    ...
    hidden = false
    sync.visibilityChanged()
    await eventually(() => assert.deepEqual(requested, [1, 1]))
    assert.equal(states.at(-1), "connected")
  } finally {
    sync.stop()
  }
...
test("a failed same-version visibility catch-up retries immediately", async () => {
  const clock = new ManualClock()
  let hidden = false
  const streams: ReadableStreamDefaultController<Uint8Array>[] = []
  const requested: number[] = []
  const sync = createWorkspaceSync({
    ...
    reconcile: async ({ minimumVersion }) => {
      requested.push(minimumVersion)
      if (requested.length === 2) throw new Error("portal unavailable")
      return { version: minimumVersion, apply: () => {} }
    },
    onStateChange: () => {},
    isHidden: () => hidden,
  ...
  try {
    ...
    await eventually(() => assert.deepEqual(requested, [1, 1, 1]))
  } finally {
    sync.stop()
  }
})
