...
23. The 26-connection reservation covers configured service bounds, one-old/one-new worker overlap, one extra instance of each request role, migration, and operator recovery. A failed reduction records pool wait and transaction latency, then retains the smallest passing capacity.
