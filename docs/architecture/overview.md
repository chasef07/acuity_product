...
2. The checked pilot reservation is 36 usable client connections: 20 across configured request-service maxima, 4 across overlapping one-instance worker revisions with two connections each, 1 migration connection, 8 for one extra instance of every request role, and 3 operator/recovery connections. Cloud Run can temporarily exceed configured maxima, so live overshoot remains a gate rather than a claimed hard ceiling.
